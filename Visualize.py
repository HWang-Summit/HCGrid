...
# show output image
plt.style.use(astropy_mpl_style)
figg = plt.figure(figsize=(8.5, 8.5))
axx = figg.add_subplot(111, projection=out_wcs.celestial)
gcii = axx.imshow(out_data, cmap='jet', vmin=out_min, vmax=out_max)
##cbarr = plt.colorbar(gcii)
##axx.set_xlabel('DEC [deg]', fontsize=15)
##axx.set_ylabel('RA [deg]', fontsize=15)
plt.show()
figg.savefig(path+'output'+num+'.png')